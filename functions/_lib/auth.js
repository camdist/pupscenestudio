...
export async function getSession(ctx){
  ...
  const row=await ctx.env.DB.prepare(`SELECT s.id session_id,s.expires_at,u.id user_id,u.email,u.name,u.role,u.status user_status,u.created_at,u.last_login_at,
      e.plan,e.status entitlement_status,e.credits_remaining,e.renewal_at,e.subscription_id
...
export function publicUser(s){return {id:s.user_id,email:s.email,name:s.name||'',role:s.role,status:s.user_status,plan:s.plan||'none',entitlementStatus:s.entitlement_status||'inactive',creditsRemaining:Number(s.credits_remaining||0),renewalAt:s.renewal_at||null,subscriptionId:s.subscription_id||null,isAdmin:s.role==='admin'&&normEmail(s.email)===ADMIN_EMAIL}}
